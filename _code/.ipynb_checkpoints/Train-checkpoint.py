...
class learn():
    ...
    def lr_scheduler(self, epoch):
        if epoch>0.8*self.num_epochs and not self.decay_time[0]: 
            self.decay_time[0] = True
            lr = self.init_lr*self.decay_rate
            print('LR is set to {}'.format(lr))
            for param_group in self.optimizer.param_groups: param_group['lr'] = lr
#         if epoch>=0.85*self.num_epochs and not self.decay_time[1]: 
...
#             for param_group in self.optimizer.param_groups: param_group['lr'] = lr
    ...
    def opt(self):
        ...
        for epoch in range(self.num_epochs+1): 
            ...
            if epoch%self.test_freq==0 or epoch in [1,3]:
        ...
        for data in dataLoader:
            ...
            with torch.set_grad_enabled(True):
                ...
                if self.global_it%self.chck_freq==0 and self.global_it>0:
                    ...
                    self.writer.add_scalar(self.Data+'HN_num', HN_num, self.global_it)
